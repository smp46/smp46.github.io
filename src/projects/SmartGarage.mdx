...
title: 'SmartGarage'
subtitle:
  'A DIY Wireless Garage Door Control System with Machine Learning Automation'
type: 'personal'
description:
...
resent it on command. The fob uses the 433MHz band for transmission. I happened
...
of the air cleanly, then resend it. Documentation for the drivers didn't back
...
something beautifully simple: the button on the fob just bridges two contacts on
...
remedy thism I made that bash script collect more often when the door might be
open and used a
...
to `/toggle` with the correct authorisation token, when receieve it triggers the
...
Super simple Python HTTP API server, using FastAPI, that receives the garage
door status from the Image Recognitio Script, updates its internal last_opened
...
open and the last opened was more than 10 minutes ago it send a friendly email
...
Component | Cost (AUD) | | :--- | ---: | | Raspberry Pi Zero W | $25.00 | | Pi
GPIO Pins and Case | $8.00 | | Generic Garage Door Fob | $8.00 | | 5V Relay |
$8.00 | | 220 Ohm 0.5 Watt Resistors | $0.85 | | Soldering Iron kit | $45.00 | |
**Total** | **$94.85** |
